# -*- coding: utf-8 -*-
"""bioimageit_core fsspec metadata service.

This module implements the fsspec service for metadata
(Data, DataSet and Experiment) management.
This service read/write and query metadata from a database
made od JSON file in a local or remote file system using the fsspec library 
...
class FsspecMetadataServiceBuilder:
    """Service builder for the metadata service"""

    def __init__(self):
        self._instance = None

    def __call__(self, file_system, host, port, username, password):
        if not self._instance:
            self._instance = FsspecMetadataService(file_system, host, port, username, password)
        return self._instance


class FsspecMetadataService:
    """Service for metadata management using the fsspec library"""

    def __init__(self, file_system, host, port, username, password):
        self.service_name = 'FsspecMetadataService'
        if file_system == 'local':
            self.fs = fsspec.filesystem('file')
        elif file_system == 'sftp':    
            self.fs = fsspec.filesystem('sftp', host=host, port=port, username=username, password=password)
        else:
            raise DataServiceError(
                f'fsspec does not recognize the file system {file_system}'
            )    
        self._sep = self.fs.sep

    def _read_json(self, md_uri: str):
        """Read the metadata from the a json file"""
        if self.fs.exists(md_uri):
            with self.fs.open(md_uri) as json_file:
                return json.load(json_file)

    def _write_json(self, metadata: dict, md_uri: str):
        """Write the metadata to the a json file"""
        with self.fs.open(md_uri, 'w') as outfile:
    ...
    def md_file_path(md_uri):
        ...
        """
        #abspath = self.abspath(md_uri)
        #return os.path.dirname(abspath)
        return os.path.dirname(md_uri)
    ...
    def absolute_path(self, file: str, reference_file: str):
    ...
    def normalize_path_sep(self, path: str) -> str:
        ...
        """
        p1 = path.replace('/', self._sep).replace('\\\\', self._sep)
        return p1
    ...
    def create_experiment(self, name, author, date='now', keys=None,
        ...
        uri = self.abspath(uri)

        # create the experiment directory
        filtered_name = name.replace(' ', '')
        experiment_path = self.join(uri, filtered_name)
        ...
        raw_data_path = self.join(experiment_path, 'data')
        raw_dataset_md_url = self.join(raw_data_path, 'raw_dataset.md.json')
        if self.fs.exists(experiment_path):
            self.fs.mkdir(raw_data_path)
        else:
            raise DataServiceError(
        ...
        container.md_uri = self.join(experiment_path, 'experiment.md.json')
    ...
    def get_workspace_experiments(self, workspace_uri):
        ...
        """
        if self.fs.exists(workspace_uri):
            #dirs = os.listdir(workspace_uri)
            dirs = self.fs.ls(workspace_uri, detail=False)
            experiments = []
            for exp_path in dirs:
                #exp_path = self.join(workspace_uri, dir, 'experiment.md.json')
                if self.fs.isdir(exp_path):
    ...
    def get_experiment(self, md_uri):
        ...
        """
        md_uri = self.abspath(md_uri)
        if self.fs.isfile(md_uri):
            ...
            raw_dataset_url = self.absolute_path(
                self.normalize_path_sep(
            ...
            for dataset in metadata['processed_datasets']:
                processed_dataset_url = self.absolute_path(
                    self.normalize_path_sep(
                        dataset['url']), md_uri)
    ...
    def update_experiment(self, experiment):
        ...
        """
        md_uri_ = self.abspath(experiment.md_uri)
        metadata = dict()
        ...
        tmp_url = self.to_unix_path(
            self.relative_path(experiment.raw_dataset.url,
                                               md_uri_))
        ...
        for dataset in experiment.processed_datasets:
            tmp_url = self.to_unix_path(
                          self.relative_path(dataset.url, md_uri_))
    ...
    def import_data(self, experiment, data_path, name, author, format_,
        ...
        """
        raw_dataset_uri = self.abspath(experiment.raw_dataset.url)
        data_dir_path = self.dirname(raw_dataset_uri)
        ...
        md_uri = self.join(data_dir_path, filtered_name + '.md.json')
        ...
        if metadata.format == 'bioformat':
            ...
            for file_ in files_to_copy:
                ...
                destination_path = self.join(data_dir_path, origin_base_name)
                #print(f'upload file from {file_} to {destination_path}')
                self.fs.upload(file_, destination_path)
            metadata.uri = self.join(data_dir_path, data_base_name)  # URI is main file
    ...
    def _import_dir_bioformat(self, raw_dataset_uri, dir_uri, filter_, author, format_, date, directory_tag_key):
        ...
        raw_dataset_uri_ = self.abspath(raw_dataset_uri)
        data_dir_path = self.dirname(raw_dataset_uri_)
    ...
    def _add_to_raw_dataset_bioformat(self, data_dir_path, raw_dataset_uri):
        ...
        tmp_file = self.join(data_dir_path, 'tmp.txt')
        if self.fs.isfile(tmp_file):
            ...
            with open(tmp_file) as file:
                ...
                for line in lines:
                    ...
                    data_md_uri = self.join(data_dir_path, line)
    ...
    def import_dir(self, experiment, dir_uri, filter_, author, format_, date,
        ...
        if directory_tag_key != '':
            key_value_pairs[directory_tag_key] = self.dirname(dir_uri)
    ...
    def get_raw_data(self, md_uri):
        ...
        """
        md_uri = self.abspath(md_uri)
        if self.fs.isfile(md_uri) and md_uri.endswith('.md.json'):
            metadata = self._read_json(md_uri)
            container = RawData()
            if 'uuid' in metadata:
                container.uuid = metadata['uuid']
            ...
            container.uri = self.absolute_path(
                self.normalize_path_sep(
            ...
            experiment_uri = self.join(Path(Path(md_uri).parent).parent, 'experiment.md.json') 
    ...
    def update_raw_data(self, raw_data):
        ...
        """
        md_uri = self.abspath(raw_data.md_uri)
        metadata = dict()
        ...
        metadata['common']['url'] = self.to_unix_path(
            self.relative_path(raw_data.uri, md_uri))
        ...
        if self.fs.isfile(md_uri) and md_uri.endswith('.md.json'):
            ...
            container.uri = self.absolute_path(
                self.normalize_path_sep(
                    metadata['common']['url']), md_uri)
            # origin run
            container.run = Container(self.absolute_path(
                self.normalize_path_sep(
    ...
    def update_processed_data(self, processed_data):
        ...
        """
        md_uri = self.abspath(processed_data.md_uri)
        metadata = dict()
        ...
        metadata['common']['url'] = self.to_unix_path(
            self.relative_path(processed_data.uri, md_uri))
        # origin type
        metadata['origin'] = dict()
        metadata['origin']['type'] = METADATA_TYPE_PROCESSED
        # run url
        run_url = self.to_unix_path(
            self.relative_path(processed_data.run.md_uri, md_uri))
        ...
        for input_ in processed_data.inputs:
            metadata['origin']['inputs'].append(
                {
                    'name': input_.name,
                    'url': self.to_unix_path(
                        self.relative_path(input_.uri, md_uri)),
                    'uuid': input_.uuid,
                    'type': input_.type,
                }
            )
        ...
        if self.fs.isfile(md_uri) and md_uri.endswith('.md.json'):
            ...
            for uri in metadata['urls']:
                container.uris.append(
                    Container(self.absolute_path(
                        self.normalize_path_sep(uri['url']),
                        md_uri),
                        uri['uuid']))
    ...
    def update_dataset(self, dataset):
        ...
        """
        md_uri = self.abspath(dataset.md_uri)
        metadata = dict()
        ...
        for uri in dataset.uris:
            #print('uri=', uri)
            tmp_url = self.to_unix_path(
                self.relative_path(uri.md_uri, md_uri))
    ...
    def create_dataset(self, experiment, dataset_name):
        ...
        experiment_md_uri = self.abspath(experiment.md_uri)
        experiment_dir = self.md_file_path(experiment_md_uri)
        dataset_dir = self.join(experiment_dir, dataset_name)
        if not self.fs.isdir(dataset_dir):
            self.fs.mkdir(dataset_dir)
        processed_dataset_uri = self.join(
        ...
        tmp_url = self.to_unix_path(processed_dataset_uri)
    ...
    def create_run(self, dataset, run_info):
        ...
        """
        # create run URI
        dataset_md_uri = self.abspath(dataset.md_uri)
        dataset_dir = self.md_file_path(dataset_md_uri)
        run_md_file_name = "run.md.json"
        run_id_count = 0
        while self.fs.isfile(self.join(dataset_dir, run_md_file_name)):
            run_id_count += 1
            run_md_file_name = "run_" + str(run_id_count) + ".md.json"
        run_uri = self.join(dataset_dir, run_md_file_name)
    ...
    def get_dataset_runs(self, dataset):
        ...
        """
        run_uri = self.join(self.dirname(dataset.md_uri), 'run.md.json')
    ...
    def get_run(self, md_uri):
        ...
        if self.fs.isfile(md_uri):
            ...
            container.process_uri = self.normalize_path_sep(
    ...
    def _write_run(self, run):
        ...
        metadata['process']['url'] = self.to_unix_path(
            run.process_uri)
        dataset_rel_url = self.to_unix_path(
            self.relative_path(run.processed_dataset.md_uri,
                               run.md_uri))
    ...
    def create_data_uri(self, dataset, run, processed_data):
        ...
        """
        md_uri = self.abspath(dataset.md_uri)
        dataset_dir = self.md_file_path(md_uri)

        extension = FormatsAccess.instance().get(processed_data.format).extension
        processed_data.uri = self.join(dataset_dir, f"{processed_data.name}.{extension}").replace('\\', '\\\\')
    ...
    def create_data(self, dataset, run, processed_data):
        ...
        data_md_file = self.join(dataset_dir, processed_data.name + '.md.json')
        processed_data.uuid = generate_uuid()
        processed_data.md_uri = data_md_file
        extension = FormatsAccess.instance().get(processed_data.format).extension
        processed_data.uri = self.join(dataset_dir, f"{processed_data.name}.{extension}")
